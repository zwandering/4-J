...
from Dataset import load_data_indp_original1
import wandb
...
def get_dataset(test_id,session): ## dataloading function, you should modify this function according to your environment setting.
    ...
    train_data_all, test_data_all = load_data_indp_original1()
    # print(test_data_all[0].data.shape, test_data_all[1].data.shape)

    train_data_all[test_id].data = train_data_all[test_id].data.reshape(train_data_all[test_id].data.shape[0], -1)
    test_data_all[test_id].data = test_data_all[test_id].data.reshape(test_data_all[test_id].data.shape[0], -1)

    # min_max_scaler = preprocessing.MinMaxScaler(feature_range=(-1, 1))
    min_max_scaler = preprocessing.StandardScaler()

    train_data_all[test_id].data = min_max_scaler.fit_transform(train_data_all[test_id].data).astype('float32')
    test_data_all[test_id].data = min_max_scaler.fit_transform(test_data_all[test_id].data).astype('float32')

    # print(train_data_all[test_id].label.unique())

    one_hot_label_mat=np.zeros((len(train_data_all[test_id].label),4)).astype('float32') # Variable 'one_hot_label_mat' is a [3394, 3] ground-truth matrix from SEED dataset.
    for i in range(len(train_data_all[test_id].label)):
        if train_data_all[test_id].label[i]==0: # '0' refers to '-1 (negative emotion)' in SEED
            one_hot_label=[1,0,0,0]
            one_hot_label=np.hstack(one_hot_label).reshape(1,4)
            one_hot_label_mat[i,:]=one_hot_label
        if train_data_all[test_id].label[i]==1: # '1' refers to '0 (neutral emotion)' in SEED
            one_hot_label=[0,1,0,0]
            one_hot_label=np.hstack(one_hot_label).reshape(1,4)
            one_hot_label_mat[i,:]=one_hot_label
        if train_data_all[test_id].label[i]==2: # '2' refers to '1 (positive emotion)' in SEED
            one_hot_label=[0,0,1,0]
            one_hot_label=np.hstack(one_hot_label).reshape(1,4)
            one_hot_label_mat[i,:]=one_hot_label
        if train_data_all[test_id].label[i]==3: # '2' refers to '1 (positive emotion)' in SEED
            one_hot_label=[0,0,0,1]
            one_hot_label=np.hstack(one_hot_label).reshape(1,4)
            one_hot_label_mat[i,:]=one_hot_label
    train_data_all[test_id].label = one_hot_label_mat

    one_hot_label_mat = np.zeros((len(test_data_all[test_id].label),
                                  4)).astype('float32')  # Variable 'one_hot_label_mat' is a [3394, 3] ground-truth matrix from SEED dataset.
    for i in range(len(test_data_all[test_id].label)):
        if test_data_all[test_id].label[i] == 0:  # '0' refers to '-1 (negative emotion)' in SEED
            one_hot_label = [1, 0, 0, 0]
            one_hot_label = np.hstack(one_hot_label).reshape(1, 4)
            one_hot_label_mat[i, :] = one_hot_label
        if test_data_all[test_id].label[i] == 1:  # '1' refers to '0 (neutral emotion)' in SEED
            one_hot_label = [0, 1, 0, 0]
            one_hot_label = np.hstack(one_hot_label).reshape(1, 4)
            one_hot_label_mat[i, :] = one_hot_label
        if test_data_all[test_id].label[i] == 2:  # '2' refers to '1 (positive emotion)' in SEED
            one_hot_label = [0, 0, 1, 0]
            one_hot_label = np.hstack(one_hot_label).reshape(1, 4)
            one_hot_label_mat[i, :] = one_hot_label
        if test_data_all[test_id].label[i] == 3:  # '2' refers to '1 (positive emotion)' in SEED
            one_hot_label = [0, 0, 0, 1]
            one_hot_label = np.hstack(one_hot_label).reshape(1, 4)
            one_hot_label_mat[i, :] = one_hot_label
    test_data_all[test_id].label = one_hot_label_mat
...
def train_model(loader_train, loader_test,model,dann_loss, optimizer,scheduler,hidden_4,epoch,batch_size,parameter,threshold_update=True): ## model training 
    ...
    for i in range(T):  
        ...
        try:
            ...
            train_source_iter=enumerate(loader_train)
            _, (x_s, labels_s) = next(train_source_iter)
...
def train_and_test_GAN(test_id,max_iter,parameter,session,threshold_update=True): ## pipeline for PR-PL model
    ...
    run = wandb.init(project=f'pr_4_19',
                     name=f'fold:{test_id}',
                     reinit=True)
    ...
    for epoch in range(max_iter):
        ...
        wandb.log({'target_acc': target_acc, 'source_acc': source_acc, 'target_nmi':target_nmi, 'source_nmi':source_nmi, 'cls_loss_sum':cls_loss_sum, 'transfer_loss_sum':transfer_loss_sum})
...
def train_prpl():
    ...
    parameter={'hidden_1':64,'hidden_2':64,'num_of_class':4,'cluster_weight':2,'low_rank':32,'upper_threshold':0.9,'lower_threshold':0.5,'boost_type':'exp'}
    ...
    # best_acc_mat,cls_loss_curve,transfer_loss_curve,source_acc_curve,source_nmi_curve,target_acc_curve,target_nmi_curve = main(False, parameter, 1)
    run = wandb.init(project=f'pr_4_19',
                       name = f'Final',
                       reinit=True)
    print(np.mean(best_acc_mat), np.std(best_acc_mat))
    wandb.log({'acc mean':np.mean(best_acc_mat), 'acc std':np.std(best_acc_mat)})
    run.finish()
